...
module SDL
  ...
  class Renderer
    ...
    def info
      ret = LibSDL.get_renderer_info(renderer, out info)
      raise Error.new("SDL_GetRendererInfo") unless ret == 0
      Info.new(info.value)
    end

    def output_size
      LibSDL.get_renderer_output_size(renderer, out w, out h)
      raise Error.new("SDL_GetRendererOutputSize") unless ret == 0
      {w, h}
    end

    def target_supported?
      ret = LibSDL.render_target_supported(renderer, texture)
    ...
    def target
      texture = LibSDL.get_render_target(renderer)
      raise Error.new("SDL_SetRenderTarget") unless ret == 0
      texture ? Texture.new(texture) : nil
    end

    def logical_size=(xy)
      ret = LibSDL.render_set_logical_size(@renderer, *xy)
      raise Error.new("SDL_RenderSetLogicalSize") unless ret == 0
      xy
    end

    def logical_size
      LibSDL.render_get_logical_size(@renderer, out x, out y)
      {x, y}
    end

    def viewport=(rect)
      ret = LibSDL.render_set_viewport(@renderer, Rect.from(rect))
      raise Error.new("SDL_RenderSetViewport") unless ret == 0
      rect
    end

    def viewport
      LibSDL.render_get_viewport(@renderer, out rect)
      Rect.from(rect)
    end

    def clip_rect=(rect)
      ret = LibSDL.render_set_clip_rect(@renderer, Rect.from(rect))
      raise Error.new("SDL_RenderSetClipRect") unless ret == 0
      rect
    end

    def clip_rect
      LibSDL.render_get_cli_rect(@renderer, out rect)
      Rect.from(rect)
    end

    def scale=(xy)
      ret = LibSDL.render_set_scale(@renderer, *xy)
      raise Error.new("SDL_RenderSetScale") unless ret == 0
      xy
    end

    def scale
      LibSDL.render_get_scale(@renderer, out x, out y)
      {x, y}
    end

    # Set the color for drawing operations (lines, rectangles and clear).
    def draw_color=(rgba)
      ret = LibSDL.set_render_draw_color(@renderer, *rgba)
    ...
    def draw_color
      ret = LibSDL.get_render_draw_color(@renderer, out r, out g, out b, out a)
    ...
    def draw_blend_mode=(blend_mode : BlendMode)
      ret = LibSDL.set_render_draw_blend_mode(@renderer, blend_mode)
    ...
    def draw_blend_mode
      ret = LibSDL.get_render_draw_blend_mode(@renderer, out blend_mode)
    ...
    def draw_point(point)
      ret = LibSDL.render_draw_point(@renderer, point.x, point.y)
      raise Error.new("SDL_RenderDrawPoint") unless ret == 0
    end

    def draw_point(x, y)
      ret = LibSDL.render_draw_point(@renderer, x, y)
      raise Error.new("SDL_RenderDrawPoint") unless ret == 0
    end

    # Draw many `Point` at once.
    def draw_points(points)
      ret = LibSDL.render_draw_points(@renderer, points, points.size)
      raise Error.new("SDL_RenderDrawPoints") unless ret == 0
    end

    # Draw a single line between two `Point`.
    def draw_line(a, b)
      ret = LibSDL.render_draw_line(@renderer, a.x, a.y, b.x, b.y)
      raise Error.new("SDL_RenderDrawLine") unless ret == 0
    end

    # Draw a single line between two `Point`.
    def draw_line(x1, y1, x2, y2)
      ret = LibSDL.render_draw_line(@renderer, x1, y2, x2, y2)
      raise Error.new("SDL_RenderDrawLine") unless ret == 0
    end

    # Draw a line between following a series of `Point`.
    def draw_lines(points)
      ret = LibSDL.render_draw_lines(@renderer, points, points.size)
      raise Error.new("SDL_RenderDrawLines") unless ret == 0
    end

    # Draw a single `Rect`.
    def draw_rect(rect)
      ret = LibSDL.render_draw_rect(@renderer, Rect.from(rect))
      raise Error.new("SDL_RenderDrawRect") unless ret == 0
    end

    # Draw a single `Rect`.
    def draw_rect(x, y, w, h)
      ret = LibSDL.render_draw_rect(@renderer, Rect.new(x, y, w, h))
      raise Error.new("SDL_RenderDrawRect") unless ret == 0
    end
    ...
    def draw_rects(rects)
      ...
      ret = LibSDL.render_draw_rects(@renderer, sdl_rects, sdl_rects.size)
      raise Error.new("SDL_RenderDrawRects") unless ret == 0
    end

    # Fill a `Rect` with the current `draw_color` and `draw_blend_mode`.
    def fill_rect(rect)
      ret = LibSDL.render_fill_rect(@renderer, Rect.from(rect))
      raise Error.new("SDL_RenderFillRect") unless ret == 0
    end

    # Fill a `Rect` with the current `draw_color` and `draw_blend_mode`.
    def fill_rect(x, y, w, h)
      ret = LibSDL.render_fill_rect(@renderer, Rect.new(x, y, w, h))
      raise Error.new("SDL_RenderFillRect") unless ret == 0
    end
    ...
    def fill_rects(rects)
      sdl_rects = rects.map { |r| Rect.from(r) }
      ret = LibSDL.render_fill_rects(@renderer, sdl_rects, sdl_rects.size)
    ...
    def copy(texture, srcrect = nil, dstrect = nil)
      LibSDL.render_copy(@renderer, texture, Rect.from(srcrect), Rect.from(dstrect))
    end
